...
async function loadPageContent(endpoint) {
	...
	// Clear all existing header cells (usually one <li>)
	headerUl.querySelectorAll('li').forEach(li => (li.innerHTML = ''));

	// Clear all previous <li> entries from the main table list body
	tableUl.innerHTML = '';

	try {
		// Fetch primary data from the given API endpoint
		// Assumes the response is an array with a single object containing .title, .intro, .items[]
		...
		if (h1) {
			if (typeof data.title === 'string' && data.title.trim()) {
				h1.textContent = data.title.trim();
			} else {
				h1.textContent = '⚠️ No title available for this section';
			}
		}
		...
		if (intro) {
			if (typeof data.intro === 'string' && data.intro.trim()) {
				intro.textContent = data.intro.trim();
			} else {
				intro.textContent = '⚠️ No description available for this section';
			}
		}

		// --- ITEM LIST (ul > li) ---
		tableUl.innerHTML = ''; // clear any old list content

		// Ensure `data.items` is a valid array; if not, attempt to re-fetch or default to empty
		if (!Array.isArray(data.items)) {
			try {
				// Retry the same endpoint in case the response is an array directly
				data.items = await fetchJSON(`${endpoint}`);
			} catch {
				// If retry fails, fallback to inline error message and halt further processing
				data.items = []; // Prevent downstream crashes by defaulting to an empty array

				// Inject structural inline error messaging (no modal)
				const article = document.querySelector('main article');
				const h1 = article?.querySelector('h1');
				const intro = article?.querySelector('p');

				if (h1) h1.textContent = '⚠️ Malformed or incomplete data';
				if (intro) intro.textContent = 'The expected item list could not be parsed from the server response.';

				// Show a fallback <li> explaining the failure
				const li = document.createElement('li');
				li.textContent = '⚠️ No list data available due to format error.';
				tableUl.innerHTML = '';
				tableUl.appendChild(li);

				// Prevent the rest of the function from continuing
				return;
			...
			if (duplicates.length) {
				const li = document.createElement('li');
				li.textContent = `⚠️ Duplicate IDs found: ${duplicates.join(', ')}`;
				tableUl.appendChild(li);
			} else {
			...
			const li = document.createElement('li');
			li.textContent = '⚠️ No item list available for this section';
			tableUl.appendChild(li);
		}
		...
		const h1 = article?.querySelector('h1');
		const intro = article?.querySelector('p');
		if (h1) h1.textContent = '⚠️ Could not load section';
		if (intro) intro.textContent = '⚠️ A network error occurred retrieving this content.';
		tableUl.innerHTML = '';
		const li = document.createElement('li');
		li.textContent = '⚠️ No item list available (fetch error)';
		tableUl.appendChild(li);
	}
}
	...
	form.onsubmit = e => {
		...
		const endpoint = document.querySelector('nav input[name="nav"]:checked');
		...
		showModal({
			...
			.then(res => {
				...
				if (res && res.ok !== false) {
					...
					loadPageContent(`${BASE_URL}${endpoint}`);
	...
	deleteButton.onclick = async () => {
		...
		try {
			...
			await loadPageContent(`${BASE_URL}${endpoint}`); // refresh the table
		} catch (err) {
