...
function clearAllTabContent() {
  ...
  articles.forEach((article) => {
    ...
    document.querySelector('main section ul').replaceChildren()
...
const fieldset = aside.querySelector(
  "form section fieldset:not([style*='display: none'])"
)
...
function loadTabContent() {
  clearAllTabContent()
  fetch("https://cdn.jsdelivr.net/gh/D7460N/DHCP@main/data/nav-content.json?nocache=timestamp")
    .then((res) => res.json())
    .then((content) => {
      const index = getActiveArticleIndex()
      const tabKeys = [
...
navInputs.forEach((input) => {
  input.onchange = () => {
    clearForm() // Clears fieldset and collapses aside
    loadTabContent() // Loads content for the selected tab
  }
})
